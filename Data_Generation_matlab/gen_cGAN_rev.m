...
%% Generating pilot 
pilot = cdm_gen_freq(pilot_len + 1, 1);
pilot = pilot(:, 1 : pilot_len).';
pilot = repmat(pilot,[1 tx_ant])';
...
%     pilot = pilot(:, 1 : pilot_len);
    ...
    s_H_real = 2 * scale_p * normalize(real(H), 'range') - scale_p;
    s_H_imag = 2 * scale_p * normalize(imag(H), 'range') - scale_p;
    
    CH(curr_dat, :, :, 1) = s_H_real;
    CH(curr_dat, :, :, 2) = s_H_imag;
    Y(curr_dat, :, :, 1) = real(y);
    Y(curr_dat, :, :, 2) = imag(y);
...
formatSpec = "Gan_%d_dBOutdoorSCM_%dpath_%dscatter_los_210611_v4.mat";
