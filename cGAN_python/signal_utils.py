...
def rayleigh_channel(
    channel_shape=None, distance_rate=None, sig_power=None, profile=None
):
    ...
    ch_coef = (
        np.random.randn(row, multipath) + 1j * np.random.randn(row, multipath)
    ) * np.sqrt(0.5)
...
def rayleigh_channel_v2(
    channel_shape=None, distance_rate=None, sig_power=None, mode=None, profile=None
):
