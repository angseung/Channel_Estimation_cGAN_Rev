...
(targetIm, inputIm) = load_image_test_y(
    "../Data_Generation_matlab/Gan_Data/Comb_3_12_25_rev.mat"
)
bit_data = np.random.randint(low=0, high=2, size=(32, bit_len))
mod_data = base_mod(bit_data, mod_scheme=mod_scheme)
...
recovered_data = base_demod(recovered_y, mod_scheme=mod_scheme)

error = np.logical_not(bit_data == recovered_data)
ber = np.mean(error, axis=None)
